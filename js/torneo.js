async function mostrarGrupos() {
    ...
    const response = await fetch( `data/posiciones.json?v=${version}` );
    ...
    .forEach(grupo => {
        ...
        grupos[grupo].forEach(
            (equipo,index) => {
                ...
                html += `
                    <tr
                        class=" fila-equipo ${clase} "
                        onclick="
                            location.href=
                            'equipo.html?equipo=' +
                            encodeURIComponent('${equipo.equipo}')
                        "
                    >
                        <td>${index+1}</td>
                        <td class="equipo-nombre"> ${obtenerBandera( equipo.equipo )} ${equipo.equipo} </td>
                        <td>${equipo.pj}</td>
...
function obtenerEquipoPorCodigo(
    ...
    return grupos[grupo][posicion - 1];
}
